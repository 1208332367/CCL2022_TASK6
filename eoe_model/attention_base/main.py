...
def parse_arguments(parser):
    ...
    parser.add_argument('--model_dir', type=str, default="model_files/chinese_model/")
    ...
    parser.add_argument('--max_no_incre', type=int, default=7,
...
def train_model(config: Config, epoch: int, train_insts=None, dev_insts=None, test_insts=None, i_fold=0):
    model = NNCRF(config)
    ...
    model_path = model_dir + f"/{model_folder}_crf_fold_{i_fold}.m"
    config_path = model_dir + f"/{model_folder}_config_fold_{i_fold}.conf"
    res_path = config.result_dir + f"{model_folder}_fold_{i_fold}.pred.json"
    log_file = model_dir + f"/{model_folder}_log.txt"
    # Create new dirs.
    ...
    for i in tqdm(range(1, epoch + 1), desc="Epoch"):
        ...
        for index in tqdm(np.random.permutation(len(batched_data))):
            sent_seq_len, sent_tensor, event_tensor, label_seq_tensor = simple_batching(config, batched_data[index],
                                                                                        tokenizer)
            model.train()
        ...
        if dev_metrics[2] > best_dev[2]:
            ...
            no_incre_dev = 0
            best_dev[0] = dev_metrics[0]  # Precision
            best_dev[1] = dev_metrics[1]  # Recall
            best_dev[2] = dev_metrics[2]  # F1
            best_epoch = i
        ...
        if no_incre_dev >= config.max_no_incre:
            ...
            break
    logging.info(f"i_fold:{i_fold}, best epoch: {best_epoch}, best dev P: {best_dev[0]}, best dev R: {best_dev[1]}, best dev F1: {best_dev[2]}")
...
def main():
    ...
    if opt.k_fold != -1:
        ...
        for i in range(k_fold):
            train, dev = get_kfold_data(k_fold, i, dataset)
            train_model(conf, conf.num_epochs, train, dev, tests, i)
    else:
