...
def train_model(config: Config, epoch: int, train_insts=None, dev_insts=None, test_insts=None):
    ...
    for i in tqdm(range(1, epoch + 1), desc="Epoch"):
        ...
        for index in tqdm(np.random.permutation(len(batched_data))):
            ...
            loss = loss / gradient_accumulation_steps # 0720 modified
            epoch_loss += loss.item() # 0720 modified
            loss.backward()
            # 0720 modified
        ...
        if dev_metrics[2] > best_dev[0]: # get each epoch result, not best
            ...
            f.close()
            # write_results(res_path.replace(f"{model_folder}.pred.json", f"epoch_{i}_{model_folder}.pred.json"), test_insts)
            write_results(res_path, test_insts) # finally get last epoch result
        else:
            no_incre_dev += 1
        print("The best dev: %.2f" % (best_dev[0]))
        print("The best dev in epoch: %d" % (best_dev[1]))
        model.zero_grad()
...
if __name__ == "__main__":
    """
    cmd1(seq_train_accum2_epoch8):nohup python eoe_model/seq_base/main.py --train_file train --gradient_accumulation_steps 2 --num_epochs 8 --model_folder seq_train_accum2_epoch8  --test_file test --lr 5e-4  --backbone_lr 1e-6  --batch_size 1 --bert hfl/chinese-macbert-base --data_dir data/ECOB-ZH/  --model_dir model_files/chinese_model/ --result_dir model_combine/task1_combine_data/ --device cuda:0 > logs/seq_train_accum2_epoch8.train.log 2>&1 &
    cmd2(seq_enhance_train_accum2_epoch5):nohup python eoe_model/seq_base/main.py --train_file enhance_train --gradient_accumulation_steps 2 --num_epochs 5 --model_folder seq_enhance_train_accum2_epoch5  --test_file test --lr 5e-4  --backbone_lr 1e-6  --batch_size 1 --bert hfl/chinese-macbert-base --data_dir data/ECOB-ZH/  --model_dir model_files/chinese_model/ --result_dir model_combine/task1_combine_data/ --device cuda:0 > logs/seq_enhance_train_accum2_epoch5.train.log 2>&1 &
    cmd3(seq_enhance_train_dev_accum2_epoch8):nohup python eoe_model/seq_base/main.py --train_file enhance_train_dev --gradient_accumulation_steps 2 --num_epochs 8 --model_folder seq_enhance_train_dev_accum2_epoch8  --test_file test --lr 5e-4  --backbone_lr 1e-6  --batch_size 1 --bert hfl/chinese-macbert-base --data_dir data/ECOB-ZH/  --model_dir model_files/chinese_model/ --result_dir model_combine/task1_combine_data/ --device cuda:0 > logs/seq_enhance_train_dev_accum2_epoch8.train.log 2>&1 &
    """
    main()
