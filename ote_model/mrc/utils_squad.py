...
ltp = LTP(path='base') 
# ltp = LTP(path='pretrained_model/ltp/base') # if not auto download, use this
...
def combine_doc_opinion(doc, opinions):
    item = {
        'doc_info': doc,
        'opinion_list': []
    }
    ...
    for opinion in opinions:
        if opinion['doc_id'] == doc_id:
            item['opinion_list'].append(opinion)
    return item

def combine_train_dev(train_file, dev_file):
    res = []
    logger.info(" start combine train and dev file……")
    train_docs = load_json(train_file + '.doc.json')
    train_opinions = load_json(train_file + '.ann.json')
    dev_docs = load_json(dev_file + '.doc.json')
    dev_opinions = load_json(dev_file + '.ann.json')
    for train_doc in train_docs:
        item = combine_doc_opinion(train_doc, train_opinions)
        res.append(item)
    for dev_doc in dev_docs:
        item = combine_doc_opinion(dev_doc, dev_opinions)
        res.append(item)
    logger.info(" finish combine train and dev file")
    return res

def save_train_dev(train_file, dev_file, train_data, dev_data):
    train_docs = []
    train_opnions = []
    dev_docs = []
    dev_opnions = []
    for item in train_data:
        train_docs.append(item['doc_info'])
        for opnion in item['opinion_list']:
            train_opnions.append(opnion)
    for item in dev_data:
        dev_docs.append(item['doc_info'])
        for opnion in item['opinion_list']:
            dev_opnions.append(opnion)
    store_json(train_file + '.doc.json', train_docs, json_indent=4)
    store_json(train_file + '.ann.json', train_opnions, json_indent=4)
    store_json(dev_file + '.doc.json', dev_docs, json_indent=4)
    store_json(dev_file + '.ann.json', dev_opnions, json_indent=4)
...
def get_boundary(event, aspect, language='english'):
    if language == 'chinese':
        start_idx = -1
        end_idx = -1
        candidate_start_idxs = [idx for idx, word in enumerate(event) if aspect[0] in word]
        candidate_end_idxs = [idx for idx, word in enumerate(event) if aspect[-1] in word]
        spans = [[s_idx, e_idx] for s_idx in candidate_start_idxs for e_idx in candidate_end_idxs if s_idx <= e_idx]
...
def read_squad_examples(input_file, language='english', opinion_level='segment', word_seg_model='jieba'):
    ...
    for idx, opinion in enumerate(opinions_reformat):  # opinion, event, aspect, event_id, doc_id, start_sent_idx, end_sent_idx
        ...
        if language == 'english':
            ...
            if word_seg_model == 'ltp': # use ltp word seg model
                doc_tokens_list, _ = ltp.seg([opinion[1].strip().replace(' ', ',')])
                doc_tokens = doc_tokens_list[0] 
            else: # use jieba word seg model(default)
                doc_tokens = list(jieba.cut(opinion[1].strip().replace(' ', ',')))
...
if __name__ == '__main__':
    res = combine_train_dev('data/ECOB-ZH/train', 'data/ECOB-ZH/dev')
    store_json('data/ECOB-ZH/combine_train_dev.json', res, json_indent=4)
    # print(res)