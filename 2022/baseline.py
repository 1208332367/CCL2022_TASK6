...
dev_dataloader = DataLoader(dev_datasets, batch_size=8, shuffle=False)
model = Model(hidden_states=256, num_layers=2, dropout=0.2, sentence_max_length=512, descriptor_max_length=20)
...
for epoch in range(num_epochs):
    ...
    total = 0
    acc = 0
    for i, (descriptors, contents, start_anns, end_anns, valid_lens, _) in enumerate(dev_dataloader):
