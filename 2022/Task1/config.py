...
class Config:
    ...
    def build_word_idx(self, train_insts: List[Instance], dev_insts: List[Instance],
        ...
        for inst in insts:
            ...
            for sent in sents:
                if sent in self.word2idx:
                    inst.sent_ids.append(self.word2idx[sent])
                else:
                    inst.sent_ids.append(self.word2idx[self.UNK])
                char_id = []
                for c in sent:
